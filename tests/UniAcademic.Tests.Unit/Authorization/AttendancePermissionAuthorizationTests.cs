...
public sealed class AttendancePermissionAuthorizationTests
{
    [Fact]
    public async Task HandleRequirementAsync_ShouldNotSucceed_WhenAttendanceEditPermissionIsMissing()
    {
        var currentUser = new FakeCurrentUser
        {
            PermissionsValue = [PermissionConstants.Attendance.View, PermissionConstants.Attendance.Create]
        };
        var handler = new PermissionAuthorizationHandler(currentUser);
        var requirement = new PermissionRequirement(PermissionConstants.Attendance.Edit);
